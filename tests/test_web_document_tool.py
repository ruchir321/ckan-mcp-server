...
from mcp_ckan_server import ckan_read_web_document
...
async def test_successful_fetch():
    ...
    try:
        # Depending on how FastMCP wraps tools, it might be the function itself or have a .fn attribute
        if hasattr(ckan_read_web_document, 'fn'):
            result = await ckan_read_web_document.fn(url)
        else:
        ...
        assert "RentSafeTO" in result or "Toronto" in result, "Result should contain expected keywords"
